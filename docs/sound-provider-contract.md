...
- `discoverCapabilities()` — discover and return the provider's supported Sound capabilities.
- `getVersion()` — report the provider adapter/API version when available.
...
Providers own provider-specific authentication, request mapping, capability discovery, and response parsing. The Sound API owns authentication, validation, credits, generation persistence, and later storage orchestration.
