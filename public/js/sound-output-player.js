(()=>{
  ...
  const resultTitle=()=>output()?.querySelector('.sound-result-top strong');
  const resultMeta=()=>output()?.querySelector('.sound-result-top span');
  ...
  const typeLabel=type=>({music:'Music',soundtrack:'Soundtrack',sfx:'SFX',ambience:'Ambience',jingle:'Jingle',loop:'Loop'})[String(type||'music').toLowerCase()]||'Sound';
  ...
  const setButton=playing=>{
    ...
    button.innerHTML=playing
      ? '<span style="width:8px;height:12px;border:0;border-left:3px solid #fff;border-right:3px solid #fff;margin:0"></span>'
      : '<span></span>';
  ...
  const updateOutputMeta=()=>{
    const duration=audio&&Number.isFinite(audio.duration)&&audio.duration>0?audio.duration:null;
    const state=stateApi()?.getState?.();
    const type=typeLabel(state?.generation?.type||'music');
    const title=resultTitle();
    const meta=resultMeta();
    if(title)title.textContent='Generated Sound';
    if(meta)meta.textContent=`${type} · ${duration!==null?formatTime(duration):formatTime(state?.output?.duration||0)}`;
    const nodes=timeNodes();
    if(nodes[1]&&duration!==null)nodes[1].textContent=formatTime(duration);
  };
  ...
  const updateTransport=()=>{
    ...
    if(Number.isFinite(duration)&&duration>0)updateOutputMeta();
  };
  ...
  const canvasSize=()=>{
    ...
    return {width:Math.max(320,Math.floor(rect.width||700)),height:Math.max(80,Math.floor(rect.height||115)),dpr:window.devicePixelRatio||1};
  };
  ...
  const drawWaveform=()=>{
    ...
    const bars=waveformData||Array.from({length:140},(_,i)=>.14+(((i*37)%83)/100)*.7);
    const gap=2.5;
    const barWidth=Math.max(1.8,(width-gap*(bars.length-1))/bars.length);
    ...
    bars.forEach((level,i)=>{
      ...
      const h=Math.max(4,level*(height-18));
      const y=(height-h)/2;
  ...
  const drawLiveWaveform=()=>{
    ...
    ctx.setTransform(dpr,0,0,dpr,0,0);
    ctx.clearRect(0,0,width,height);
    const base=waveformData||Array.from({length:140},(_,i)=>.14+(((i*37)%83)/100)*.7);
    ...
    for(let i=0;i<barCount;i++){
      const x=i*(barWidth+gap);
      const freqIndex=Math.min((analyserData?.length||1)-1,Math.floor((i/barCount)*(analyserData?.length||1)));
      const live=analyserData?.length?analyserData[freqIndex]/255:0;
      const phase=Math.sin((performance.now()/170)+(i*.38))*.06;
      const pulse=.78+live*.72+phase;
      const h=Math.max(4,Math.min(height-10,base[i]*(height-18)*pulse));
      const y=(height-h)/2;
    ...
    try{
      ...
      try{
        ...
        for(let i=0;i<barCount;i++){
          ...
          peaks.push(Math.max(peak*.78,rms*3.2));
        }
        const max=Math.max(...peaks,.01);
        waveformData=peaks.map(value=>Math.min(1,Math.max(.045,value/max)));
      }finally{
  ...
  const ensureAudio=()=>{
    ...
    audio.addEventListener('timeupdate',()=>{
      updateTransport();
      if(!audio.paused)drawLiveWaveform();
    });
  ...
  const wireDownload=()=>{
    ...
    download.addEventListener('click',async event=>{
      ...
      try{
        ...
        if(!filename.includes('.'))filename+=`.${(stateApi()?.getState?.()?.output?.format||'mp3').toLowerCase()}`;
        ...
        setPlayerError(String(error?.message||'Sound download failed').slice(0,300));
      }finally{
  ...
  const showOutput=data=>{
    ...
    const actualDuration=data.durationSeconds||data.result?.durationSeconds||null;
    const format=data.format||data.result?.format||'mp3';
    const mimeType=data.mimeType||data.result?.mimeType||'audio/mpeg';
    const size=data.sizeBytes||data.result?.sizeBytes||null;

    applyingOutput=true;
    out.setOutput({status:'ready',assetId:generationId,r2Key:currentR2Key,format,mimeType,duration:actualDuration,size});
  ...
  const generateSound=async event=>{
    ...
    try{
      ...
      const response=await window.fetch('/api/sound/generate',{method:'POST',headers:{'content-type':'application/json'},credentials:'same-origin',body:JSON.stringify(payload)});
      const data=await response.json().catch(()=>null);
      console.log('svara_sound_generate_response',{status:response.status,data});
      if(!response.ok){
        failGeneration(data?.generationId||null,String(data?.error||'Sound generation request failed.').slice(0,300));
        return;
      }
      if(!data?.id){
    ...
    if(button){
      button.addEventListener('click',async event=>{
        ...
        try{
          ...
          setPlayerError('Sound playback could not start.');
        }
      },true);
    }
  };
  ...
  const syncFromState=state=>{
    ...
    showOutput({id:state.output.assetId,r2Key:state.output.r2Key,format:state.output.format,mimeType:state.output.mimeType,durationSeconds:state.output.duration,sizeBytes:state.output.size});
  };
