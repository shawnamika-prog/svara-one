(()=>{
  ...
  function styles(){
    ...
    style.textContent=`
      #soundWorkspace .sound-sf-voice-context{margin:0 2px 10px;padding:10px 11px;border:1px solid #a85cff26;border-radius:12px;background:linear-gradient(145deg,#0d1321,#09111d);box-shadow:inset 0 0 24px #9b63ff06}
      ...
      @media(max-width:560px){#soundWorkspace .sound-sf-voice-context-actions{align-items:stretch;flex-direction:column}.sound-sf-voice-context-action{width:100%}}
    `;
  ...
  function render(voice){
    const r=root();
    const composer=r?.querySelector('.sound-sf-composer');
    const head=r?.querySelector('.sound-sf-head');
    if(!composer||!head)return;
    remove();
