...
async function playStoredSample(voice,button){
 ...
 if(stopHeroPlayback)stopHeroPlayback();
...
function setupHeroPlayer(){
 ...
 const track=document.querySelector('.hero-card .bar');
 ...
 if(!button||!track||!bar)return;
 let heroAudio=null,started=false,audioContext=null,analyser=null,source=null,animationFrame=null,seeking=false;
 ...
 const setRemainingTime=()=>{if(!duration||!heroAudio||!Number.isFinite(heroAudio.duration))return;duration.textContent=formatDuration(Math.max(0,heroAudio.duration-heroAudio.currentTime));};
 const updateProgress=()=>{if(!heroAudio||!Number.isFinite(heroAudio.duration)||heroAudio.duration<=0)return;bar.style.width=`${heroAudio.currentTime/heroAudio.duration*100}%`;setRemainingTime();};
 const seekFromPointer=(event)=>{if(!heroAudio||!Number.isFinite(heroAudio.duration)||heroAudio.duration<=0)return;const rect=track.getBoundingClientRect();const ratio=Math.max(0,Math.min(1,(event.clientX-rect.left)/rect.width));heroAudio.currentTime=ratio*heroAudio.duration;updateProgress();};
 ...
 stopHeroPlayback=()=>{if(!heroAudio)return;heroAudio.pause();heroAudio.currentTime=0;reset();started=false;};
 track.setAttribute('role','slider');track.setAttribute('tabindex','0');track.setAttribute('aria-label','Seek voice preview');track.addEventListener('click',event=>{if(event.target===button)return;seekFromPointer(event);});
 track.addEventListener('pointerdown',event=>{if(!heroAudio||!Number.isFinite(heroAudio.duration))return;seeking=true;track.setPointerCapture?.(event.pointerId);seekFromPointer(event);});
 track.addEventListener('pointermove',event=>{if(seeking)seekFromPointer(event);});
 track.addEventListener('pointerup',event=>{seeking=false;track.releasePointerCapture?.(event.pointerId);});
 track.addEventListener('pointercancel',()=>{seeking=false;});
 track.addEventListener('keydown',event=>{if(!heroAudio||!Number.isFinite(heroAudio.duration))return;let delta=0;if(event.key==='ArrowLeft')delta=-5;if(event.key==='ArrowRight')delta=5;if(event.key==='Home')heroAudio.currentTime=0;if(event.key==='End')heroAudio.currentTime=heroAudio.duration;if(delta){event.preventDefault();heroAudio.currentTime=Math.max(0,Math.min(heroAudio.duration,heroAudio.currentTime+delta));updateProgress();}else if(event.key==='Home'||event.key==='End'){event.preventDefault();updateProgress();}});
 button.addEventListener('click',async()=>{try{if(heroAudio&&started){if(heroAudio.paused){if(activeAudio){activeAudio.pause();activeAudio.currentTime=0;activeAudio=null;resetSampleButtons();}if(audioContext)await audioContext.resume();await heroAudio.play();}else heroAudio.pause();return;}if(activeAudio){activeAudio.pause();activeAudio.currentTime=0;activeAudio=null;resetSampleButtons();}button.disabled=true;button.textContent='…';heroAudio=new Audio('/api/sample-hero');heroAudio.preload='auto';heroAudio.addEventListener('loadedmetadata',()=>{if(duration&&Number.isFinite(heroAudio.duration))duration.textContent=formatDuration(heroAudio.duration);});heroAudio.addEventListener('timeupdate',()=>{if(!seeking)updateProgress();});heroAudio.addEventListener('play',async()=>{started=true;button.disabled=false;button.textContent='Ⅱ';await connectAnalyser();if(!animationFrame)animateWave();});heroAudio.addEventListener('pause',()=>{if(started){button.textContent='▶';stopWave();}});heroAudio.addEventListener('ended',()=>{reset();started=false;});heroAudio.addEventListener('error',()=>{reset();console.error('Hero sample unavailable');});await heroAudio.play();}catch(err){reset();console.error(err);}});
 resetWave();
}
