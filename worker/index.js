...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/health") {
      return json({ ok: true, service: "svara-origins-api", version: "2", providers: getProviderStatus(env) }, 200, request);
    }
    ...
    if (url.pathname === "/api/lab/catalogue" && request.method === "GET") {
      ...
      try {
        ...
        return json({ provider: "deepgram", tests: LAB_TESTS.map(([id]) => id), voices: voices.map(v => ({ voice_id: v.canonical_name, metadata: v.metadata || {} })) }, 200, request);
      } catch (err) {
        return json({ error: String(err?.message || "Catalogue failed").slice(0, 300) }, 502, request);
      }
    }
      ...
      try {
        ...
        for (const voiceId of voices) {
          for (const [testId] of LAB_TESTS) {
            try {
              samples.push({ status: "generated", ...(await generateLabSample(env, voiceId, testId)) });
            } catch (err) {
              samples.push({ status: "error", voice_id: voiceId, test_id: testId, error: String(err?.message || "Generation failed").slice(0, 300) });
            }
          }
        }
        ...
        return json({ error: String(err?.message || "Lab batch failed").slice(0, 300) }, 502, request);
      }
    }
    ...
    if (url.pathname === "/api/voice/generate" && request.method === "POST") {
      try {
        ...
        const upstream = await provider.generate({ text, voice, speed: Number(body.speed) || 1, stability: Number(body.stability) || 50, style: String(body.style || "") });
