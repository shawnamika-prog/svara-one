...
export default {
  async fetch(request, env) {
    ...
    // Keep the Studio available at both /studio and /studio.html.
    if (url.pathname === "/studio") {
      return env.ASSETS.fetch(new Request(new URL("/studio.html", request.url), request));
    }

    // The API is the only route that must run through Worker code first.
