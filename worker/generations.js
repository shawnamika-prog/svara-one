...
export async function markGenerationReady(env, id, r2Object) { await env.DB.prepare(`UPDATE generations SET status='ready',r2_etag=?,size_bytes=?,completed_at=strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE id=?`).bind(r2Object?.etag||null,Number.isFinite(Number(r2Object?.size))?Number(r2Object.size):null,id).run(); }
export async function markGenerationFailed(env, id, status='failed') { const safeStatus=status==='storage_failed'?'storage_failed':'failed'; await env.DB.prepare(`UPDATE generations SET status=?,completed_at=strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE id=?`).bind(safeStatus,id).run(); }
...
export async function deleteUserGenerationAudio(env,userId){if(!env.DB||!env.GENERATED_AUDIO)return;const rows=await env.DB.prepare("SELECT r2_key FROM generations WHERE user_id=? AND r2_key IS NOT NULL").bind(userId).all();const keys=(rows.results||[]).map(row=>String(row.r2_key||'')).filter(Boolean);if(keys.length)await env.GENERATED_AUDIO.delete(keys);}
export async function cleanupExpiredGenerations(env,limit=100){if(!env.DB||!env.GENERATED_AUDIO)return {checked:0,deleted:0};const rows=await env.DB.prepare(`SELECT id,r2_key FROM generations WHERE expires_at<=strftime('%Y-%m-%dT%H:%M:%fZ','now') ORDER BY expires_at ASC LIMIT ?`).bind(Math.max(1,Math.min(1000,Number(limit)||100))).all();let deleted=0;for(const row of rows.results||[]){try{if(row.r2_key)await env.GENERATED_AUDIO.delete(row.r2_key);await env.DB.prepare('DELETE FROM generations WHERE id=?').bind(row.id).run();deleted++;}catch(error){console.error('generation_cleanup_error',row.id,error);}}return {checked:(rows.results||[]).length,deleted};}
function sessionToken(request){const cookie=request.headers.get('Cookie')||'';for(const part of cookie.split(';')){const index=part.indexOf('=');if(index===-1)continue;if(part.slice(0,index).trim()===SESSION_COOKIE)return decodeURIComponent(part.slice(index+1).trim());}return '';}
async function authenticatedUserId(request,env){if(!env.DB)return null;const token=sessionToken(request);if(!token)return null;const digest=await crypto.subtle.digest('SHA-256',new TextEncoder().encode(token));const tokenHash=btoa(String.fromCharCode(...new Uint8Array(digest))).replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,'');const row=await env.DB.prepare(`SELECT u.id FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.token_hash=? AND s.revoked_at IS NULL AND s.expires_at>strftime('%Y-%m-%dT%H:%M:%fZ','now') AND u.status='active' LIMIT 1`).bind(tokenHash).first();return row?.id||null;}
function json(data,status=200){return new Response(JSON.stringify(data),{status,headers:{'content-type':'application/json; charset=utf-8','cache-control':'no-store'}});}
async function listUserGenerationObjects(env,userId,requestedLimit){if(!env.GENERATED_AUDIO)throw new Error('Generated audio R2 storage is not configured.');const prefix=`users/${userId}/generations/`,limit=Math.max(1,Math.min(500,requestedLimit)),objects=[];let cursor;while(objects.length<limit){const page=await env.GENERATED_AUDIO.list({prefix,limit:Math.min(1000,limit-objects.length),...(cursor?{cursor}:{})});objects.push(...(page.objects||[]));if(!page.truncated||!page.cursor)break;cursor=page.cursor;}return objects.slice(0,limit);}
