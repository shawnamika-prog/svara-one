...
export async function cleanupExpiredGenerations(env,limit=100){
  if(!env.DB||!env.GENERATED_AUDIO)return {checked:0,deleted:0,soundChecked:0,soundDeleted:0};
  const safeLimit=Math.max(1,Math.min(1000,Number(limit)||100));
  const rows=await env.DB.prepare(`SELECT id,r2_key FROM generations WHERE expires_at<=strftime('%Y-%m-%dT%H:%M:%fZ','now') ORDER BY expires_at ASC LIMIT ?`).bind(safeLimit).all();
  let deleted=0;
  for(const row of rows.results||[]){
    try{
      if(row.r2_key)await env.GENERATED_AUDIO.delete(row.r2_key);
      await env.DB.prepare('DELETE FROM generations WHERE id=?').bind(row.id).run();
      deleted++;
    }catch(error){console.error('generation_cleanup_error',row.id,error);}
  }

  await env.DB.prepare(`UPDATE sound_generations SET expires_at=datetime(created_at, '+${SOUND_RETENTION_DAYS} days') WHERE expires_at IS NULL AND created_at IS NOT NULL`).run();
  const soundRows=await env.DB.prepare(`SELECT id,r2_key FROM sound_generations WHERE expires_at<=strftime('%Y-%m-%dT%H:%M:%fZ','now') ORDER BY expires_at ASC LIMIT ?`).bind(safeLimit).all();
  let soundDeleted=0;
  for(const row of soundRows.results||[]){
    try{
      if(row.r2_key)await env.GENERATED_AUDIO.delete(row.r2_key);
      await env.DB.prepare('DELETE FROM sound_generations WHERE id=?').bind(row.id).run();
      soundDeleted++;
    }catch(error){console.error('sound_generation_cleanup_error',row.id,error);}
  }
  return {checked:(rows.results||[]).length,deleted,soundChecked:(soundRows.results||[]).length,soundDeleted};
}
...
async function listUserSoundObjects(env,userId,requestedLimit){if(!env.GENERATED_AUDIO)throw new Error('Generated audio R2 storage is not configured.');const prefix=`users/${userId}/sound/`,limit=Math.max(1,Math.min(500,requestedLimit)),objects=[];let cursor;while(objects.length<limit){const page=await env.GENERATED_AUDIO.list({prefix,limit:Math.min(1000,limit-objects.length),...(cursor?{cursor}:{})});objects.push(...(page.objects||[]));if(!page.truncated||!page.cursor)break;cursor=page.cursor;}return objects.slice(0,limit);}
