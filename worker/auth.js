...
async function currentUser(request, env) {
  ...
  const voiceRows = await env.DB.prepare(`
    SELECT voice_id
    FROM user_voices
  ...
  return userPayload(
    row,
    subscription,
    Number(balanceRow?.balance_after || 0),
    (voiceRows.results || []).map(item => item.voice_id)
  );
}
...
async function login(request, env) {
  ...
  const user = await env.DB.prepare(`
    SELECT id, email, display_name, status, created_at, password_hash, password_salt
    FROM users
  ...
  const payload = await currentUser(freshRequest, env);
  return json({ ok: true, user: payload }, 200, request, { "Set-Cookie": sessionCookie(sessionToken) });
}
...
async function me(request, env) {
  ...
  if (!user) return json({ authenticated: false, user: null }, 200, request);
  return json({ authenticated: true, user }, 200, request);
}
